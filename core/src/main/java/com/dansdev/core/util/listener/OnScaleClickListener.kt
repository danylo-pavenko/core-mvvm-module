...
abstract class OnScaleClickListener(
    ...
    private fun setVisibleRect() {
        ...
        view?.get()?.getLocationOnScreen(location)
        val width = view?.get()?.width.orZero()
        val height = view?.get()?.height.orZero()
        val x = location[0]
        val y = location[1]
...
abstract class OnShortScaleClickListener : OnScaleClickListener(debounceTime = 300, scale = 0.98f)

fun View.setOnClickWithTouchImpact(clickAction: () -> Unit) {
    this.setOnTouchListener(object : OnShortScaleClickListener() {
